...
export async function GET(request: Request) {
  ...
  const objectSize = ["s", "m", "l"].includes(searchParams.get("objectSize") ?? "") ? searchParams.get("objectSize")! : "m";
  ...
  let parts;
  try {
    parts = datePartsInZone(timeZone);
  } catch {
    parts = datePartsInZone("UTC");
  }
    ...
    modules.push(
      <div key="year" style={{ display: "flex", flexDirection: "column", width: "100%", gap: Math.round(14 * scale) }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", width: "100%" }}>
          <div style={{ fontSize: small, letterSpacing: "0.14em" }}>{lang === "ru" ? "ПРОГРЕСС ГОДА" : "YEAR PROGRESS"}</div>
          <div style={{ fontSize: medium, letterSpacing: "-0.03em" }}>{display}</div>
        </div>
        <div style={{ width: "100%", height: Math.max(5, Math.round(7 * scale)), border: `${line}px solid ${fg}`, display: "flex" }}>
          <div style={{ width: `${progress}%`, height: "100%", background: fg }} />
        </div>
      </div>
    );
  }
  ...
  if (objectMode) {
    ...
    modules.push(
      <div key="object" style={{ width: "100%", display: "flex", flexDirection: "column", alignItems: "center", gap: Math.round(18 * scale) }}>
        <PixelSprite matrix={matrix} color={fg} px={pxMap[objectSize]} />
        {details && (
          <div style={{ fontSize: Math.round(11 * scale), letterSpacing: "0.16em", opacity: 0.68 }}>
  ...
  if (birthday !== "off") {
    ...
    modules.push(
      <div key="birthday" style={{ display: "flex", width: "100%", alignItems: "baseline", justifyContent: "space-between", borderTop: details ? `${line}px solid ${fg}` : undefined, paddingTop: details ? Math.round(18 * scale) : 0 }}>
        <div style={{ fontSize: small, letterSpacing: "0.14em" }}>{lang === "ru" ? "ДО ДНЯ РОЖДЕНИЯ" : "UNTIL BIRTHDAY"}</div>
  ...
  if (motivationEnabled) {
    ...
    modules.push(
      <div key="motivation" style={{ width: "100%", display: "flex", flexDirection: "column", gap: Math.round(10 * scale) }}>
        {details && <div style={{ fontSize: Math.round(11 * scale), letterSpacing: "0.16em", opacity: 0.65 }}>{lang === "ru" ? "// СЛОВО_ДНЯ_" : "// WORD_OF_THE_DAY_"}</div>}
        <div style={{ fontSize: Math.round(72 * scale), lineHeight: 0.9, letterSpacing: "-0.055em", fontWeight: 700 }}>{word}</div>
      </div>
    );
  }

  const count = modules.length;
  const justify = count <= 2 ? "space-around" : "space-between";
  ...
  return new ImageResponse(
    (
      ...
      >
        {details && (
          <>
            <div style={{ position: "absolute", left: padX, top: Math.round(device.height * 0.07), fontSize: Math.round(12 * scale), letterSpacing: "0.18em", opacity: 0.6 }}>// DAILY_SYSTEM_</div>
            <div style={{ position: "absolute", right: padX, top: Math.round(device.height * 0.07), fontSize: Math.round(12 * scale), letterSpacing: "0.18em", opacity: 0.6 }}>LOCK / {device.width}×{device.height}</div>
            <div style={{ position: "absolute", left: padX, right: padX, top: Math.round(device.height * 0.235), height: line, background: fg, opacity: 0.18 }} />
            <div style={{ position: "absolute", left: padX, bottom: Math.round(device.height * 0.035), fontSize: Math.round(10 * scale), letterSpacing: "0.18em", opacity: 0.5 }}>
              {String(doy).padStart(3, "0")} / {parts.year}
            </div>
            <div style={{ position: "absolute", right: padX, bottom: Math.round(device.height * 0.035), fontSize: Math.round(10 * scale), letterSpacing: "0.18em", opacity: 0.5 }}>
              + {String(spriteIndex + 1).padStart(2, "0")}
            </div>
          </>
        )}
        ...
        <div
          style={{
            ...
            right: padX,
            top: safeTop,
            bottom: Math.round(device.height * 0.08),
            display: "flex",
        ...
        >
          {modules.length ? modules : (
            <div style={{ fontSize: Math.round(18 * scale), opacity: 0.5, letterSpacing: "0.16em" }}>// EMPTY_SYSTEM_</div>
          )}
        </div>
      </div>
    ),
